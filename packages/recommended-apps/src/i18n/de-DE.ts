import type { RecommendedAppsTranslations } from './types';

export const DE_DE: RecommendedAppsTranslations = {
  TITLE: 'Unsere Apps',
  OPEN_PLAY_STORE: 'Im Play Store ansehen',
  EMPTY_STATE: 'Keine empfohlenen Apps verfügbar',
  APP_NAME_CCFK: 'Cover Creator für Kindle',
  APP_NAME_ECC: 'EPUB Cover ändern',
  APP_NAME_PCM: 'PDF-Cover erstellen',
  APP_DESC_CCFK: 'Bild zu Kindle-Cover oder direkt per Scratch starten',
  APP_DESC_ECC: 'EPUB-Cover in Sekunden ändern',
  APP_DESC_PCM: 'PDF-Cover schnell und lokal erstellen',
  APP_NAME_EF: 'EPUB reparieren',
  APP_DESC_EF: 'Häufige EPUB-Probleme prüfen und reparierte Kopie speichern',
};
