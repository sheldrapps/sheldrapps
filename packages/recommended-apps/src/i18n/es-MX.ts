import type { RecommendedAppsTranslations } from './types';

export const ES_MX: RecommendedAppsTranslations = {
  TITLE: 'Nuestras apps',
  OPEN_PLAY_STORE: 'Ver en Play Store',
  EMPTY_STATE: 'No hay apps recomendadas disponibles',
  APP_NAME_CCFK: 'Crear Portadas para Kindle',
  APP_NAME_ECC: 'Cambiar portada a EPUB',
  APP_NAME_PCM: 'Creador de Portadas PDF',
  APP_DESC_CCFK: 'Convierte imagen en portada Kindle o empieza desde scratch',
  APP_DESC_ECC: 'Cambia la portada de tu EPUB en segundos',
  APP_DESC_PCM: 'Crea portadas PDF en segundos',
  APP_NAME_EF: 'Reparar EPUB',
  APP_DESC_EF: 'Detecta y repara problemas comunes en archivos EPUB',
  APP_NAME_EMAS: 'Unir y dividir EPUB',
  APP_DESC_EMAS: 'Une, organiza y divide archivos EPUB',
};
