import type { RecommendedAppsTranslations } from './types';

export const AR_SA: RecommendedAppsTranslations = {
  TITLE: 'تطبيقاتنا',
  OPEN_PLAY_STORE: 'عرض على Play Store',
  EMPTY_STATE: 'لا توجد تطبيقات موصى بها',
  APP_NAME_CCFK: 'إنشاء أغلفة لـ Kindle',
  APP_NAME_ECC: 'تغيير غلاف EPUB',
  APP_NAME_PCM: 'صانع غلاف PDF',
  APP_DESC_CCFK: 'حوّل صورة الى غلاف Kindle أو ابدأ من scratch',
  APP_DESC_ECC: 'غيّر غلاف أي ملف EPUB خلال ثوانٍ',
  APP_DESC_PCM: 'أنشئ غلاف PDF بسرعة وعلى جهازك',
  APP_NAME_EF: 'إصلاح EPUB',
  APP_DESC_EF: 'تشخيص وإصلاح مشاكل EPUB الشائعة',
  APP_NAME_EMAS: 'دمج وتقسيم EPUB',
  APP_DESC_EMAS: 'ادمج ملفات EPUB أو قسّمها على جهازك',
};
