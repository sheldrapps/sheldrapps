...
export class MyPdfsPage implements OnInit, OnDestroy {
  ...
  private async renameByFilename(
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        initialFilename: filename.replace(/\.pdf$/i, ''),
    ...
    try {
      const renamed = await this.files.renameGeneratedPdf({
        from: filename,
        to: data.trim(),
      });
  ...
  private async openProjectByFilename(filename: string | null): Promise<void> {
    ...
    try {
      ...
      const navigated = await this.router.navigate(['/tabs/change'], {
        queryParams: { project: filename, editMode },
      });
      if (!navigated) {
        this.loading = false;
