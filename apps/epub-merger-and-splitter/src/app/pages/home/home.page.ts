...
import {
  ...
  EpubFixerPortError as EpubMergerAndSplitterPortError,
  EpubRewriteError,
  EpubRepairResult,
...
import { EpubMergerAndSplitterWorkflowService } from '../../services/epub-merger-and-splitter-workflow.service';
import {
  EpubLibraryService,
...
export class HomePage implements OnInit, OnDestroy {
  private readonly workflow = inject(EpubMergerAndSplitterWorkflowService);
  ...
  private async requestRewardedAdForFix(): Promise<boolean> {
    ...
    try {
      ...
      console.warn('[epub-merger-and-splitter] rewarded ad gate failed', error);
      const accepted = await this.openAdFallbackFromFailure({
        rewardEarned: false,
        adClosed: false,
        failed: true,
  ...
  private async exportCurrentCopy(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn('[epub-merger-and-splitter] save exported epub failed', saveError);
      }
  ...
  private mapPrepareError(error: unknown): string {
    if (
      error instanceof EpubMergerAndSplitterPortError &&
  ...
  private failWorkflow(errorKey: string, error?: unknown): void {
    ...
    this.viewState = 'failed';
    if (error) {
      console.error('[epub-merger-and-splitter] workflow action failed', error);
    }
  }
