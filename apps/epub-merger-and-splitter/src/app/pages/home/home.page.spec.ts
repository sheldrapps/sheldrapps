...
describe('HomePage', () => {
  it('diagnoses automatically after preparing an EPUB', async () => {
    ...
    await HomePage.prototype.onEpubSelected.call(ctx, {
      target: input,
    } as unknown as Event);
  ...
  it('fixes and saves a copy after a rewarded ad on the free flow', async () => {
    ...
    await HomePage.prototype.runRepair.call(ctx);
  ...
  it('shows the Fix action for an ambiguous OPF diagnosis and waits for a guided choice', () => {
    ...
    HomePage.prototype.onGuidedOptionChange.call(
      ctx,
    ...
    expect(
      HomePage.prototype.selectedGuidedOption.call(ctx, ctx.diagnosis.issues[0]),
    ).toBe('OPS/alt.opf');
  });
  ...
  it('requires confirmation checkboxes before repairing confirmation issues', () => {
    ...
    HomePage.prototype.onConfirmationChange.call(
      ctx,
      ctx.diagnosis.issues[0],
      true,
    );
    ...
    expect(
      HomePage.prototype.isConfirmationChecked.call(ctx, ctx.diagnosis.issues[0]),
    ).toBeTrue();
    expect(canRepairDescriptor?.get?.call(ctx)).toBeTrue();
  });
  ...
  it('toggles confirmation selection from the choice row handler', () => {
    ...
    expect(
      HomePage.prototype.isConfirmationChecked.call(ctx, issue),
    ).toBeFalse();

    HomePage.prototype.toggleConfirmation.call(ctx, issue);
  ...
  it('shows the remove ads entry point only while ads are active', () => {
    ...
    expect(HomePage.prototype.canShowRemoveAdsEntryPoint.call(ctx)).toBeTrue();

    ctx.adsRemoved = true;

    expect(HomePage.prototype.canShowRemoveAdsEntryPoint.call(ctx)).toBeFalse();
  ...
  it('opens the remove ads modal after preparing the billing UI', async () => {
    ...
    await HomePage.prototype.openPurchaseModal.call(ctx);
  ...
  it('forwards a preferred OPF path to the repair workflow', async () => {
    ...
    await HomePage.prototype.runRepair.call(ctx, 'OPS/package.opf');
  ...
  it('uses the guided OPF selection when fixing an ambiguous package', async () => {
    ...
    await HomePage.prototype.onPrimaryAction.call(ctx);

    const selectionKey = HomePage.prototype.issueSelectionKey.call(
      ctx,
      issue,
    );
  ...
  it('forwards guided selections for internal link repairs', async () => {
    ...
    HomePage.prototype.onGuidedOptionChange.call(ctx, issue, 'OPS/img/cover.png');

    await HomePage.prototype.runRepair.call(ctx);

    const selectionKey = HomePage.prototype.issueSelectionKey.call(ctx, issue);
  ...
  it('localizes known issue details while keeping technical paths intact', () => {
    ...
    const translated = HomePage.prototype.issueDetailsLabel.call(ctx, {
      code: 'OPF_MISSING',
      severity: 'error',
      fixable: false,
    ...
    const passthrough = HomePage.prototype.issueDetailsLabel.call(ctx, {
      code: 'LINK_TARGET_MISSING',
      severity: 'warning',
      fixable: true,
      messageKey: 'FIX.ISSUE_LINK_TARGET_MISSING',
  ...
  it('classifies manifest items as automatic repairs', () => {
    expect(
      HomePage.prototype.issueRepairMode.call(Object.create(HomePage.prototype), {
        code: 'MANIFEST_ITEM_MISSING',
        severity: 'warning',
        fixable: true,
        messageKey: 'FIX.ISSUE_MANIFEST_ITEM_MISSING',
  ...
  it('treats unreadable zips as recoverable severity', () => {
    ...
    expect(
      HomePage.prototype.issueSeverityLevel.call(ctx, {
        code: 'ZIP_UNREADABLE',
        severity: 'error',
        fixable: true,
        messageKey: 'FIX.ISSUE_ZIP_UNREADABLE',
  ...
  it('treats unresolved internal link issues as blocked', () => {
    ...
    const ctx = Object.create(HomePage.prototype);

    expect(HomePage.prototype.issueRepairMode.call(ctx, issue)).toBe(
      'not_repairable',
    );
  });
  ...
  it('treats a fixable empty spine as a review issue', () => {
    ...
    expect(HomePage.prototype.issueRepairMode.call(ctx, issue)).toBe('review');
  });
  ...
  it('opens a saved EPUB project back in the fix flow', async () => {
    ...
    const opened = await HomePage.prototype[
