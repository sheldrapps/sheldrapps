...
function runCategoryContractSuite(platform: 'android' | 'web'): void {
  describe(`CategoryRepository (${platform})`, () => {
    let previousTimeoutInterval: number;
    let repository: CategoryRepository;

    beforeAll(() => {
      previousTimeoutInterval = jasmine.DEFAULT_TIMEOUT_INTERVAL;
      // IndexedDB setup/cleanup can be slower on CI and local Windows runs.
    ...
    beforeEach(async () => {
      ...
      if (platform === 'web') {
        // CI/Windows can leave IndexedDB locked across specs; use the explicit
        // in-memory fallback path to keep parity assertions deterministic.
        (repository as unknown as { browserFallbackToMemory: boolean }).browserFallbackToMemory =
          true;
      }
    });
    ...
    it('reports a storage mode', () => {
      ...
      if (platform === 'android') {
        ...
        expect(['browser-indexeddb', 'memory-debug']).toContain(mode);
      }
    });
  });
}
