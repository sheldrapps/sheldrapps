...
export class HomePageComponent implements OnInit {
  ...
  readonly playStoreIconSrc = `data:image/svg+xml;base64,${btoa(LOGO_GOOGLE_PLAYSTORE_ICON)}`;
  ...
  selectedLanguage: SupportedHomeLocale = 'en-US';
  ...
  selectLanguage(code: SupportedHomeLocale): void {
  ...
  private applyLanguage(language: SupportedHomeLocale, persist: boolean): void {
  ...
  private normalizeLanguage(value: string): SupportedHomeLocale {
    switch (value) {
      case 'es-MX':
      case 'de-DE':
      case 'fr-FR':
