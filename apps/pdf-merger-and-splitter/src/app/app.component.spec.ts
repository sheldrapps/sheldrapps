...
describe('AppComponent', () => {
  ...
  beforeEach(() => {
    ...
    translateInstantSpy = jasmine
      .createSpy('instant')
      .and.returnValue('PDF Merger & Splitter');
  });
  ...
  it('updates the document title on startup and language changes', async () => {
    ...
    expect(titleSetSpy).toHaveBeenCalledWith('PDF Merger & Splitter');

    translateInstantSpy.and.returnValue('Combinador PDF');
    onLangChange.next({});

    expect(titleSetSpy).toHaveBeenCalledWith('Combinador PDF');
  });
});
