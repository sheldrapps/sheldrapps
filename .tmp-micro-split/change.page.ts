...
export class ChangePage implements OnInit, OnDestroy {
  ...
  private adFallback = inject(AdFallbackService);
  ...
  private readonly adFallbackTotal = 10;
    ...
    try {
      if (!this.billing.isAdsRemoved()) {
        if (this.adFallbackTrialActive && this.resolveAdFallbackRemaining() > 0) {
          ...
          if (result.failed) {
            const accepted = await this.openAdFallbackFromFailure(result);
            if (!accepted) {
              await this.showToast(
                'CHANGE.ADS_REQUIRED',
                { duration: 1800 },
